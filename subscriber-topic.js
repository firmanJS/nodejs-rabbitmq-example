...
amqp.connect(amqpUrl).then(function(conn) {
  ...
  return conn.createChannel().then(function(ch) {
    var ok = ch.assertExchange(exchange_name, 'topic', {
      durable: true
    });

    const okQueue = ch.assertQueue('hello-assertqueue', { durable: false })
    okQueue.then(() => {
          return ch.consume('hello-assertqueue', msg => console.log('1- Received', msg.content.toString()), { noAck: true })
      })
    ...
    return ok.then(function() {
      console.log('Subscriber 1 Waiting for messages delivered to queue: ' + queue_name);
    });
